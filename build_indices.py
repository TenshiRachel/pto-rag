...
def build_faiss_index(chunked_docs, embedding_model):
    ...
    if os.path.exists("faiss_index/index.faiss"):
        print("[CACHE] Loading existing FAISS index...")
        faiss_store = FAISS.load_local(
            "faiss_index",
            embeddings=embedding_model,
            allow_dangerous_deserialization=True
        )
        print("[CACHE] FAISS index loaded from disk")
    else:
        print("[BUILD] Building new FAISS index...")
        # FAISS Vector Index
        faiss_store = FAISS.from_documents(chunked_docs, embedding_model)
        faiss_store.save_local("faiss_index")
        print("[BUILD] FAISS index built and saved")
